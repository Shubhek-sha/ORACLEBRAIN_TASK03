...
export default function StockCard({ stock }) {
  const { selectedStock, fetchStockDetail, isFavorite, toggleFavorite } = useStock();
  ...
  const handleToggle = useCallback(async (e) => {
    ...
    setToggling(true);
    try {
      await toggleFavorite(stock.symbol, stock.name);
    } finally {
      setToggling(false);
    }
  }, [isAuthenticated, toggling, toggleFavorite, stock.symbol, stock.name]);

  return (
    <div
      onClick={() => fetchStockDetail(stock.symbol)}
      className={`card p-4 cursor-pointer transition-all duration-150 hover:shadow-md hover:-translate-y-0.5 active:scale-[0.98] ${flashClass} ${
      ...
      <div className="mt-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-xs text-slate-400">
          ...
          <button
            ...
            className={`transition-all active:scale-110 disabled:cursor-default
              ${toggling ? 'opacity-50' : ''}
              ${isFav
